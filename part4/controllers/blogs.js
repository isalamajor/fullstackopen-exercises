...
blogRouter.delete("/:id", requireAuth, async (request, response) => {
  try {
    ...
    if (!blog.user || !blog.user.equals(user.id)) {
      return response
        .status(401)
        .json({ error: "Only its creator is authorized to delete a blog" });
    }
...
/* For part 7 */
blogRouter.put("/:id/comments", requireAuth, async (request, response) => {
  try {
    ...
    const res = await Blog.findByIdAndUpdate(
      id,
      { comments: newComments }, // solo se actualizan estos campos
      { new: true }
    ).populate("user");
    if (res) return response.status(200).json(res);
    return response.status(400).end();
