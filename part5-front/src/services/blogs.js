...
const getAll = async () => {
  try {
    ...
    return { ok: false, error: error.response?.data.error || 'Blogs Fetching Failed' }
  }
}
...
const postBlog = async newObject => {
  try {
    ...
    return { ok: false, error: error.response?.data.error || 'Blog Posting Failed' }
  }
}
...
const updateBlog = async blog => {
  try {
    ...
    return { ok: false, error: error.response?.data.error || 'Blog Updating Failed' }
  }
}
...
const deleteBlog = async blogId => {
  try {
    ...
    return { ok: false, error: error.response?.data.error || 'Blog Deleting Failed' }
  }
}
